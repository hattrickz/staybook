...
export default function RegisterPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (gqlErrors?.length) {
        toast.error(gqlErrors[0].message || 'Registration failed')
        return
      }
  ...
  return (
    <div className="min-h-screen bg-surface-secondary flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md">
        <Link href={'/' as Route} className="flex items-center gap-2 font-semibold text-lg text-ink-primary mb-8">
        ...
        <div className="card p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <input {...register('name')} type="text" placeholder="John Smith" className="input" autoComplete="name" />
          ...
          <p className="text-center text-xs text-ink-tertiary mt-4">
            ...
            <Link href={'/terms' as Route} className="text-brand hover:underline">Terms</Link>{' '}
            and{' '}
            <Link href={'/privacy' as Route} className="text-brand hover:underline">Privacy Policy</Link>.
          </p>
        </div>

        <p className="text-center text-sm text-ink-secondary mt-4">
          Already have an account?{' '}
          <Link href={'/auth/login' as Route} className="text-brand font-medium hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  )
